...
public class TechdleGameCreationTests(CustomWebApplicationFactory factory) : BaseIntegrationTest(factory)
{
	...
	private async Task SeedPuzzleAsync(DateOnly date, Guid languageId)
	{
		using var scope = Factory.Services.CreateScope();
		var dbContext = scope.ServiceProvider.GetRequiredService<AppDbContext>();
		var puzzle = new DailyTechdle(date, languageId);
	...
	private async Task<DateOnly?> GetLatestPuzzleDateAsync()
	{
		using var scope = Factory.Services.CreateScope();
		var dbContext = scope.ServiceProvider.GetRequiredService<AppDbContext>();
		return await dbContext.DailyTechdles
			.AsNoTracking()
	...
	private async Task<DailyTechdle?> GetPuzzleAsync(Guid puzzleId)
	{
		using var scope = Factory.Services.CreateScope();
		var dbContext = scope.ServiceProvider.GetRequiredService<AppDbContext>();
		return await dbContext.DailyTechdles
	...
	public async Task CreateGame_AdminWithLanguages_Returns201CreatedWithPuzzleData()
	{
		...
		var response = await adminClient.PostAsync("/api/v1/techdle/games", null);
		var content = await response.Content.ReadFromJsonAsync<SuccessApiResponse<CreateTechdleGameResponseDto>>();

		Assert.Equal(HttpStatusCode.Created, response.StatusCode);
		Assert.NotNull(content);
		Assert.True(content!.Success);
		Assert.Equal("Techdle puzzle generated successfully.", content.Message);
	...
	public async Task CreateGame_NonAdminUser_Returns403Forbidden()
	{
		...
		var response = await userClient.PostAsync("/api/v1/techdle/games", null);
	...
	public async Task CreateGame_AnonymousUser_Returns401Unauthorized()
	{
		...
		var response = await Client.PostAsync("/api/v1/techdle/games", null);
	...
	public async Task CreateGame_DuplicateDateConcurrentRequest_FirstSucceeds_SecondConflicts()
	{
		...
		var task1 = adminClient1.PostAsync("/api/v1/techdle/games", null);
		var task2 = adminClient2.PostAsync("/api/v1/techdle/games", null);
		...
		if (response1.StatusCode == HttpStatusCode.Created && response2.StatusCode == HttpStatusCode.Created)
		{
			...
			var content1 = await response1.Content.ReadFromJsonAsync<SuccessApiResponse<CreateTechdleGameResponseDto>>();
			var content2 = await response2.Content.ReadFromJsonAsync<SuccessApiResponse<CreateTechdleGameResponseDto>>();
