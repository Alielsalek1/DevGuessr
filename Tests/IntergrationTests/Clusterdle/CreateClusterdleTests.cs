...
public class CreateClusterdleTests(CustomWebApplicationFactory factory) : BaseIntegrationTest(factory)
{
    ...
    public async Task Create_WithValidData_Returns201Created()
    {
        ...
        var (response, content, json) = await TechnectionCategoriesTestHelpers.CreateAsync<SuccessApiResponse<CreateClusterdleResponseDto>>(client, request);
    ...
    public async Task Create_WithDuplicateGroupName_Returns409Conflict()
    {
        ...
        var (initialResponse, _, _) = await TechnectionCategoriesTestHelpers.CreateAsync<SuccessApiResponse<CreateClusterdleResponseDto>>(client, request);
